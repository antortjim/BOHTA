...
As it can be seen in the plot, the ERa and ERb ChiP sites are both overlapping with the same chromosomes, being these 1, 3, 6, 21, 22, and X. In addition, if we consider the merging information, the binding fraction is more or less the same for both receptors (Figure 4). Moreover, when we uploaded the ERa and ERb .bed files as custom track to the UCSC browser, many of the sites resulted to be shared between the two receptors. A biological explanation for observing several overlapping ChiP sites for ERa and ERb could be that these receptors not only form homodimers, but also heterodimers with each other (https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3389841/). Another possible explanation could be that ERa and ERb have similar or even contradicting actions on some of the same genes. A more basic technical explanation for getting so similar result for both receptors could be that in the laboratory process, the antibody used for detecting ERa and ERb in the ChiP analysis was not specific for either of them.
Another noticeably thing is, that according to the data set, ERa and ERb only bind to 6 out of 22 chromosomes. It seems highly unlikely that these receptors only bind to these 6 chromosomes, so this data set must be a result of some kind of filtration of data or maybe we only see results from these 6 chromosomes because of a technicality of the tilling assay.
...
As shown in Figure 5, there are 236 ERa ChIP intervals that do not overlap with Erb ones. On the contrary, there are 140 ERb ChIP sites that do not overlap with ERa ones. Finally, ERa and ERb have 345 overlapping CHiP intervals
