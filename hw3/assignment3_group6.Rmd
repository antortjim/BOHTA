...
```{r, echo = F, include = F}
line_count <- matrix(c(824551, 824538, 824558, 824558,
                       824611, 824538, 824573, 824545),
                     byrow = T, nrow = 4,
...
Lines of the combined GTF file (combined transcriptome): 1082433

The similarity between the number of lines in the individual and the combined transcriptomes suggests that most of the exons and RNA isoforms present in  the one for HOXA1 KD cells. Therefore it makes sense to perform a differential expression analysis on the combined transcriptome.

![Capture of the chr1:1,000,000-1,500,00 region displaying the regions in the genome where transcripts were assembled by cufflinks (cuffmerge output). The guide transcriptome is displayed in the Albin track \ref{fig:bigbed}. Cuffmerge output was submitted as a bigbed file obtained from applying gtf2bed to the cuffmerge output and subsequently compressing it with the bedToBigBed utility.](bigbed.png)
...
Regardless of performing the process taking out the NaNs rows (6) or keeping them as 0, we obtain a mean and a median very close to 0, as seen in the figure \ref{fig:histogram}. This agrees with an expected mean dIF of 0 since adding up all  the dIF values for the different isoforms of the same gene is 0 by definition (although the median does not have to be 0). However, we are not obtaining a 0 mean due to numerical errors in R
...
```{r, echo = 1, fig.width = 3, fig.height = 2}
...
# Order the data frame by p values in ascending order,
# select the gene_id column, make the ids unique and select the first one.
# This gene_id is that with the lowest p value
# Filter the data frame by selecting the rows where gene_id is this id

# Once you filter like this, select the column transcript_id
