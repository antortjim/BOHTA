...
We need to test the difference of the means of both distributions. The Welch two sample t-test may be used if a normal distribution can be assumed. If the assumption of normality is doubtful, the Wilcoxon test (non-parametric) should be used. For this reason, the first step is to test the normality of our distributions. Although it can be checked by eye at the histograms in Figure 3 that both the lengths of introns and exons do not follow a normal distribution, a more detailed Q-Q plot between the observed lengths and the theoretical normal distribution has been constructed.
...
```
**Figure 3** _Q-Q plots of exon and intron lengths against their corresponding theoretical normal distributions. Both plots show 
deviations from the abline, which represents great differences between the theoretical normal distributions and the distributions
that both the lengths of introns and exons are actually following._
...
Our null hypothesis is that there is no difference in the U-statistic of the exons and introns lengths. In contrast, our alternative hypothesis is that there is no difference in the U-statistic of the exons and introns lengths.
...
**Figure 4** _Scatterplot of the studied variables showing regression lines and Pearson correlation coefficient. The cofficient sign (+/-) and range (from -1 to +1) indicates the direction of monotonicity and degree of linearity of the two variables under question, respectively._
